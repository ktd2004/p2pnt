...
PJ_DEF(pj_status_t) pj_turn_srv_create(pj_pool_factory *pf, pj_turn_srv **p_srv)
{
    ...
    /* Array of listeners */
    srv->core.listener = (pj_turn_listener**) pj_pool_calloc(pool, MAX_LISTENERS, 
																...
																unsigned addr_len)
{
    ...
    status = pj_stun_session_create_res(sess, rdata, code, 
					(errmsg?pj_cstr(&reason,errmsg):NULL),&tdata);
    if (status != PJ_SUCCESS)
		return status;
...
static void handle_binding_request (pj_turn_pkt *pkt,unsigned options)
{
    ...
    /* Decode request */
    status = pj_stun_msg_decode(pkt->pool, pkt->pkt, pkt->len, options, &request, NULL, NULL);
    if (status != PJ_SUCCESS)
		  return;

    /* Create response */
    status = pj_stun_msg_create_response(pkt->pool, request, 0, NULL, &response);
    if (status != PJ_SUCCESS)
		  return;
    ...
    /* Send response */
    pkt->transport->sendto(pkt->transport, pdu, len, 0, &pkt->src.clt_addr, pkt->src_addr_len);
}
...
PJ_DEF(void) pj_turn_srv_on_rx_pkt(pj_turn_srv *srv, pj_turn_pkt *pkt)
{
    ...
    pj_lock_acquire(srv->core.lock);
    alloc = (pj_turn_allocation*) pj_hash_get(srv->tables.alloc, &pkt->src, sizeof(pkt->src), NULL);
    pj_lock_release(srv->core.lock);
    ...
    if (alloc) {
			...
			if (status != PJ_SUCCESS) 
			{
				...
				if ((*pkt->pkt != 0x00 && *pkt->pkt != 0x01) ||
						pkt->len > 1600 || (options & PJ_STUN_IS_DATAGRAM)) 
				{
