/* $Id: listener_tcp.c 2394 2008-12-23 17:27:53Z bennylp $ */
/* 
												...
												pj_turn_listener **p_listener)
{
    ...
    /* Init bind address */
    status = pj_sockaddr_init(af, &tcp_lis->base.addr, bound_addr, (pj_uint16_t)port);
    if (status != PJ_SUCCESS) 
		goto on_error;
    ...
    status = pj_ioqueue_register_sock(pool, srv->core.ioqueue, 
									  tcp_lis->base.sock, tcp_lis, &ioqueue_cb, &tcp_lis->key);

    /* Create op keys */
    tcp_lis->accept_op = (struct accept_op*)
		pj_pool_calloc(pool, concurrency_cnt, sizeof(struct accept_op));

    /* Create each accept_op and kick off read operation */
    for (i=0; i<concurrency_cnt; ++i) {
		lis_on_accept_complete(tcp_lis->key, 
							   &tcp_lis->accept_op[i].op_key, PJ_INVALID_SOCKET, PJ_EPENDING);
    }

    /* Done */
    PJ_LOG(4,(tcp_lis->base.obj_name, "Listener %s created", tcp_lis->base.info));
...
static pj_status_t lis_destroy(pj_turn_listener *listener)
{
    ...
    if (tcp_lis->base.pool) {
		...
		PJ_LOG(4,(tcp_lis->base.obj_name, "Listener %s destroyed", tcp_lis->base.info));
								 ...
								 pj_ssize_t bytes_read)
{
    ...
    do {
		...
		if (bytes_read > 0) {
			...
			if (tcp->alloc) {
				if (bytes_read != 0) {
					show_err(tcp->base.obj_name, "TCP socket error", -bytes_read);
				} else {
