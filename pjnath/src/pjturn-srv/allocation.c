/* $Id: allocation.c 2394 2008-12-23 17:27:53Z bennylp $ */
/* 
																							...
																							pj_turn_allocation **p_alloc)
{
    ...
    status = pj_lock_create_recursive_mutex(pool, alloc->obj_name, &alloc->lock);
    if (status != PJ_SUCCESS) {
			goto on_error;
    }
    ...
    /* Print info */
    pj_ansi_strcpy(alloc->info, pj_turn_tp_type_name(transport->listener->tp_type));
    alloc->info[3] = ':';
    ...
    pj_stun_session_set_credential(alloc->sess, PJ_STUN_AUTH_LONG_TERM, &alloc->cred);
																...
																pj_turn_relay_res *relay)
{
    ...
    /* Save realm */
    sa = (pj_stun_string_attr*) pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_REALM, 0);
    PJ_ASSERT_RETURN(sa, PJ_EINVALIDOP);
    pj_strdup(pool, &relay->realm, &sa->value);

    /* Save username */
    sa = (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_USERNAME, 0);
    PJ_ASSERT_RETURN(sa, PJ_EINVALIDOP);
    ...
    for (retry=0; retry<retry_max; ++retry) {
		...
		if (status != PJ_SUCCESS) {
			...
			PJ_LOG(4,(THIS_FILE, "Unable to allocate relay, giving up: err %d", status));
			pj_sock_close(relay->tp.sock);
			relay->tp.sock = PJ_INVALID_SOCKET;
			return status;
		}
		...
		if (!pj_sockaddr_has_addr(&relay->hkey.addr)) {
			pj_sockaddr_copy_addr(&relay->hkey.addr, &alloc->transport->listener->addr);
		}
		...
		if (status != PJ_SUCCESS) {
			PJ_LOG(4,(THIS_FILE, "pj_ioqueue_register_sock() failed: err %d", status));
			pj_sock_close(relay->tp.sock);
			relay->tp.sock = PJ_INVALID_SOCKET;
			return status;
		}
													...
													const pj_stun_rx_data *rdata)
{
    ...
    if (alloc->relay.lifetime != 0) {
		...
		if (PJ_STUN_GET_METHOD(tdata->msg->hdr.type)!=PJ_STUN_CHANNEL_BIND_METHOD){
			pj_stun_msg_add_uint_attr(tdata->pool, 
										tdata->msg, PJ_STUN_ATTR_LIFETIME, interval);
													...
													unsigned addr_len)
{
    ...
    perm = (pj_turn_permission*) pj_hash_get(alloc->peer_table, 
    ...
    if (is_stun) {
			...
			if (pkt->transport->listener->tp_type == PJ_TURN_TP_UDP) {
				...
				if (parsed_len == pkt->len) {
					...
					pj_memmove(pkt->pkt, pkt->pkt+parsed_len, pkt->len - parsed_len);
					pkt->len -= parsed_len;
				}
			}
														...
														const pj_sockaddr *src_addr)
{
    ...
    /* Lookup permission */
    perm = lookup_permission_by_addr(alloc, src_addr, pj_sockaddr_get_len(src_addr));
    if (perm == NULL) {
																				 ...
																				 unsigned src_addr_len)
{
    ...
    peer_attr = 
			(pj_stun_peer_addr_attr*) pj_stun_msg_find_attr (msg, PJ_STUN_ATTR_PEER_ADDR, 0);
    ...
    /* Get DATA attribute */
    data_attr = (pj_stun_data_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_DATA, 0);
