...
static int init()
{
    ...
    for (i=0; i<(int)PJ_ARRAY_SIZE(g.peer); ++i) {
			...
			const pj_str_t s = pj_str ("192.168.10.7");
			pj_sockaddr_in_init (&addr, &s, 0);
			pj_stun_sock_cfg_default (&cfg);
																		 ...
																		 pj_status_t status)
{
    ...
    if (status == PJ_SUCCESS) {
		  ...
		  pj_stun_sock_op_name(op)));
			...
			char errmsg[PJ_ERR_MSG_SIZE];
			pj_strerror(status, errmsg, sizeof(errmsg));
			PJ_LOG(1,(THIS_FILE, "peer%d: %s error: %s", peer-g.peer, pj_stun_sock_op_name(op), errmsg));
