					    ...
					    pj_stun_nat_detect_cb *cb)
{
    ...
    PJ_ASSERT_RETURN(stun_cfg->pf && stun_cfg->ioqueue && stun_cfg->timer_heap,PJ_EINVAL);

    /*
     * Init NAT detection session.
     */
    pool = pj_pool_create(stun_cfg->pf, "natck%p", PJNATH_POOL_LEN_NATCK, PJNATH_POOL_INC_NATCK, NULL);
    if (!pool)
	return PJ_ENOMEM;
    ...
    status = pj_sock_bind(sess->sock, &sess->local_addr, sizeof(pj_sockaddr_in));
    if (status != PJ_SUCCESS)
		goto on_error;
					...
					unsigned src_addr_len)
{
    ...
    if (status == PJ_SUCCESS) {
		...
		if (PJ_STUN_IS_ERROR_RESPONSE(response->hdr.type)) {
			...
			if (mattr == NULL) {
				mattr = (pj_stun_sockaddr_attr*) pj_stun_msg_find_attr(response, PJ_STUN_ATTR_MAPPED_ADDR, 0);
			}
    ...
    if (test_id >= ST_MAX) {
		PJ_LOG(4,(sess->pool->obj_name, "Invalid transaction ID %u in response",test_id));
		end_session(sess, PJ_STATUS_FROM_STUN_CODE(PJ_STUN_SC_SERVER_ERROR), PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
		goto on_return;
    }

    PJ_LOG(5,(sess->pool->obj_name, "Completed %s, status=%d",test_names[test_id], status));

    sess->result[test_id].complete = PJ_TRUE;
    sess->result[test_id].status = status;
    if (status == PJ_SUCCESS) {
		pj_memcpy(&sess->result[test_id].ma, &mattr->sockaddr.ipv4,sizeof(pj_sockaddr_in));
		pj_memcpy(&sess->result[test_id].ca, &ca->sockaddr.ipv4,sizeof(pj_sockaddr_in));
    }
    ...
    {
		cmp = pj_memcmp(&sess->local_addr, &sess->result[ST_TEST_1].ma,sizeof(pj_sockaddr_in));
		if (cmp != 0)
   ...
   Symmetric  N  /  \       +--------+   N  /  \                   V
	...
	if (cmp==0) {
		 ...
		 */
		end_session(sess, sess->result[ST_TEST_2].status, PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
		break;
	    }
	} else {
	    /*
		...
		case PJ_SUCCESS:
		     ...
		     */
		    cmp = pj_memcmp(&sess->result[ST_TEST_1].ma, &sess->result[ST_TEST_1B].ma, sizeof(pj_sockaddr_in));
		    if (cmp != 0) {
			/*
		...
		case PJNATH_ESTUNTIMEDOUT:
			 ...
			 */
			end_session(sess, PJ_SUCCESS, PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
			break;
		    }
		    break;
		default:
		     ...
		     */
		    end_session(sess, sess->result[ST_TEST_1B].status, PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
		    break;
		}
		break;
	    default:
	 ...
	 */
	end_session(sess, sess->result[ST_TEST_1].status, PJ_STUN_NAT_TYPE_ERR_UNKNOWN);
	break;
    }
