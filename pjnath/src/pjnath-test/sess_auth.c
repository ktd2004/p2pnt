			   ...
			   int (*more_check)(void))
{
    ...
    /* Create request */
    status = pj_stun_session_create_req(client->sess, PJ_STUN_BINDING_REQUEST, PJ_STUN_MAGIC, NULL, &tdata);
    if (status != PJ_SUCCESS) {
	    destroy_client_server();
	    return -250;
    }
    ...
    if (expected_error) {
	    ...
	    if (client->response_status != 0) {
	        PJ_LOG(3,(THIS_FILE, "    err: expecting successful operation but got error %d", client->response_status));
	        rc = -600;
	        goto done;
	    } 
	    ...
	    if (PJ_STUN_IS_ERROR_RESPONSE(client->response->hdr.type)) {
	        ...
	        aerr = (pj_stun_errcode_attr*)pj_stun_msg_find_attr(client->response, PJ_STUN_ATTR_ERROR_CODE, 0);
	        if (aerr == NULL) {
