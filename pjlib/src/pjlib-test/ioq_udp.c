...
static int compliance_test(pj_bool_t allow_concur)
{
    ...
    callback_read_key = callback_write_key = callback_accept_key = callback_connect_key = NULL;
...
static int many_handles_test(pj_bool_t allow_concur)
{
    ...
    for (count=0; count<MAX; ++count) {
		...
		if (rc != PJ_SUCCESS || sock[count] == PJ_INVALID_SOCKET) {
			PJ_LOG(3,(THIS_FILE, "....unable to create %d-th socket, rc=%d", count, rc));
			break;
		}
		key[count] = NULL;
		rc = pj_ioqueue_register_sock(pool, ioqueue, sock[count], NULL, &test_cb, &key[count]);
		if (rc != PJ_SUCCESS || key[count] == NULL) {
			PJ_LOG(3,(THIS_FILE, "....unable to register %d-th socket, rc=%d", count, rc));
			return -30;
		}
    }
...
static int bench_test(pj_bool_t allow_concur, int bufsize, int inactive_sock_count)
{
    ...
    for (i=0; i<inactive_sock_count; ++i) {
		...
		rc = pj_ioqueue_register_sock(pool, ioque, inactive_sock[i], NULL, &test_cb, &keys[i]);
		if (rc != PJ_SUCCESS) {
			pj_sock_close(inactive_sock[i]);
			inactive_sock[i] = PJ_INVALID_SOCKET;
    ...
    for (i=0; i<LOOP; ++i) {
		...
		// Starts send on the client side.
			bytes = bufsize;
		rc = pj_ioqueue_sendto(ckey, &write_op, send_buf, &bytes, 0,&addr, sizeof(addr));
		if (rc != PJ_SUCCESS && rc != PJ_EPENDING) {
		...
		if (callback_read_size != bufsize || pj_memcmp(send_buf, recv_buf, bufsize)) 
		{
			rc = -10;
